...
async function initDB() {
	...
	try {
		...
		await db.pool.query('CREATE INDEX IF NOT EXISTS "idx__searchtopic__content_trgm" ON "searchtopic" USING GIN ("content" gin_trgm_ops)');
		await db.pool.query('CREATE INDEX IF NOT EXISTS "idx__searchpost__content_trgm" ON "searchpost" USING GIN ("content" gin_trgm_ops)');
		await db.pool.query('CREATE INDEX IF NOT EXISTS "idx__searchchat__content_trgm" ON "searchchat" USING GIN ("content" gin_trgm_ops)');
	} catch (err) {
		winston.warn(`[nodebb-plugin-dbsearch] failed to enable pg_trgm substring indexes: ${err.message}`);
	}
}
...
exports.search = async function (key, data, limit) {
	const useSubstringFallback = shouldUseSubstringFallback(data.content);

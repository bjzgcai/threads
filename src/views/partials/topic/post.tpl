...
{{{ end }}}
<article class="d-flex align-items-start gap-3 post-container-parent zgci-post-card">
	<div class="bg-body d-none d-sm-block rounded-circle zgci-post-avatar">
	...
	<div class="post-container d-flex gap-2 flex-grow-1 flex-column w-100" style="min-width:0;">
		...
		<div class="content text-break zgci-post-content" component="post/content" itemprop="text">
			{posts.content}
		</div>
		...
		<div component="post/footer" class="post-footer pb-2">
			...
			<div class="d-flex flex-wrap-reverse gap-2 {{{ if (hideReplies || !posts.replies.count) }}}justify-content-end{{{ else }}}justify-content-between{{{ end }}}">
				...
				<div component="post/actions" class="d-flex flex-grow-1 align-items-center justify-content-end gap-1 post-tools">
					...
					<a component="post/reply" href="#" class="btn btn-ghost btn-sm {{{ if !privileges.topics:reply }}}hidden{{{ end }}}" title="[[topic:reply]]" aria-label="[[topic:reply]]"><i class="fa fa-fw fa-reply text-primary"></i></a>
					<a component="post/quote" href="#" class="btn btn-ghost btn-sm {{{ if !privileges.topics:reply }}}hidden{{{ end }}}" title="[[topic:quote]]" aria-label="[[topic:quote]]"><i class="fa fa-fw fa-quote-right text-primary"></i></a>
					...
					<div class="d-flex votes align-items-center">
						<a component="post/upvote" href="#" class="btn btn-ghost btn-sm{{{ if posts.upvoted }}} upvoted{{{ end }}}" title="[[topic:upvote-post]]" aria-label="[[topic:upvote-post]]">
							<i class="fa fa-fw fa-chevron-up text-primary"></i>
						</a>

						<meta itemprop="upvoteCount" content="{posts.upvotes}">
						<meta itemprop="downvoteCount" content="{posts.downvotes}">
						<a href="#" class="px-2 mx-1 btn btn-ghost btn-sm" component="post/vote-count" data-votes="{posts.votes}" title="[[global:voters]]" aria-label="[[global:voters]]">{posts.votes}</a>

						{{{ if !downvote:disabled }}}
						<a component="post/downvote" href="#" class="btn btn-ghost btn-sm{{{ if posts.downvoted }}} downvoted{{{ end }}}" title="[[topic:downvote-post]]" aria-label="[[topic:downvote-post]]">
